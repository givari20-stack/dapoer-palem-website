...
export function AboutPresentation({ content, contact }: { content: PublicAboutContent | null; contact: AboutContact }) {
  ...
  const hasIdentity = Boolean(content.brand_identity_heading || content.brand_identity_description || content.audience_heading || content.audience_description);
  ...
  return (
    <main className="overflow-hidden bg-cream pt-28 text-dark-green sm:pt-32">
      ...
      {hasIdentity ? <section className="bg-brand-white py-24 sm:py-32"><Container className="grid gap-12 lg:grid-cols-2 lg:gap-20"><EditorialText heading={content.brand_identity_heading} description={content.brand_identity_description} eyebrow="Brand Identity" /><EditorialText heading={content.audience_heading} description={content.audience_description} eyebrow="Audience" /></Container></section> : null}

      {content.offerings_heading || content.offerings_items.length ? <ItemSection eyebrow="Offerings" heading={content.offerings_heading} items={content.offerings_items} /> : null}
      {content.service_channels_heading || content.service_channels_items.length ? <ItemSection eyebrow="Service Channels" heading={content.service_channels_heading} items={content.service_channels_items} links /> : null}
